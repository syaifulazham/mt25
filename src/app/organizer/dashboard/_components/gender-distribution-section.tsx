...
type GenderStateData = Record<string, number>;
...
export default function GenderDistributionSection() {
  // Changed to store formatted data directly - the chart expects this format
      ...
      try {
        ...
        if (Array.isArray(result) && result.length > 0) {
          ...
          if (transformedData.MALE > 0 || transformedData.FEMALE > 0) {
            ...
            setError('No valid gender distribution data available');
          }
        } else {
          // If we got an empty or invalid data format
          console.warn('Received invalid data format', result);
          setError('No gender distribution data available');
        }
      } catch (err) {
