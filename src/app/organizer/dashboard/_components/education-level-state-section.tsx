...
export default function EducationLevelStateSection({ stateId }: { stateId: string }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch(`/api/dashboard/education-level?stateId=${stateId}`);
        
        if (!response.ok) {
          throw new Error(`Failed to fetch education level data: ${response.status}`);
        }
        ...
        setError('Failed to load education level data');
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <CardDescription>Distribution by contestant education level</CardDescription>
      </CardHeader>
          ...
          <TabsContent value="chart" className="h-[250px]">
            {loading ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-muted-foreground">
                <p>No education level data available</p>
              </div>
            ) : (
              <div className="h-full flex flex-col justify-between">
                <div className="flex-1 space-y-2">
                  {sortedData.map((item, index) => {
                    ...
                    const formattedLevel = formatEducationLevel(item.educationLevel);
                    
                    return (
                      <div key={index} className="space-y-1">
                        <div className="flex items-center justify-between">
                          <div className="flex-1 truncate" title={formattedLevel}>
                            <span className="text-sm font-medium">{formattedLevel}</span>
                          </div>
                    ...
                    {sortedData.map((item, index) => {
                      ...
                      return (
                        <tr key={index} className="border-b">
                          <td className="p-2 text-sm" title={formattedLevel}>{formattedLevel}</td>
