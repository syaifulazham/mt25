...
export default function AssignContestsModal({ 
  ...
  const fetchEligibleContests = async () => {
    try {
      ...
      const response = await fetch(`/api/organizer/contingents/${contingentId}/contestants/${contestantId}/assign-contests`, {
  ...
  const handleSave = async () => {
    try {
      ...
      toast.success(
        "Contests Updated", 
        { description: `Successfully updated contest assignments for ${contestantName}` }
      );
      ...
      <Dialog open={isOpen} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Select the contests this contestant should participate in.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4">
            {isFetching ? (
              ...
              <>
                {contestant && (
                  <div className="mb-4">
                    <p className="text-sm font-medium">Age: {contestant.age ?? 'Not specified'}</p>
                  </div>
                )}
