...
import { 
  ...
  Legend, ResponsiveContainer, PieChart, Pie, Cell, AreaChart, Area, LineChart, Line 
} from 'recharts';
import { 
  ArrowLeft, Download, FileText, BarChart as BarChartIcon, 
  PieChart as PieChartIcon, ChevronRight 
} from 'lucide-react';
...
export function SurveyResultsView({ survey, onBack }: SurveyResultsViewProps) {
  ...
  type FiltersState = {
    ...
    education: string[],
    state: string[],
    answers: Record<string, string> // Add answer filters (e.g., {'q1': 'Yes', 'q3': 'Option A'})
  };
  ...
  // Function to get filtered responses data based on active filters
  const getFilteredData = () => {
    // If no demographic filters are active and no answer filters, return all data
    if (!Object.values(filters).some((arr: any) => Array.isArray(arr) && arr.length > 0) && 
    ...
    return (results?.rawData || []).filter((respondent: any) => {
      // Check demographic filter types
      ...
      if (filters.age.length > 0) {
        const ageGroup = getAgeGroupFromValue(respondent.age);
  ...
  const applyFilters = (data: any[], filterType: FilterTypes) => {
    // If no filters are selected, return all data
    if (filters[filterType].length === 0) return data;
    
    // Otherwise, filter the data
    return data.filter(item => filters[filterType].includes(item.name));
  };
  ...
  const handleQuestionSelect = (questionId: number) => {
    ...
    const question = results?.questions?.find((q: any) => q.questionId === questionId);
    if (question) {
      setSelectedQuestion(questionId);
      console.log('Selected question:', question);
      
      // Get answers just for this question from filtered respondents
      const questionAnswers = filteredResponses.filter((r: any) => 
        r.questionId === questionId
      );
      
      // Count distribution based on filtered data
      const answerDistribution: Record<string, number> = {};
      
      questionAnswers.forEach((answer: any) => {
        let processedAnswer: string;
        
        if (question.type === 'multiple_choice' && typeof answer.answer === 'string') {
          // Split multiple choice answers and count each separately
          const choices = answer.answer.split(',').map((c: string) => c.trim());
        ...
        // Count for single choice or text
      ...
      if (question.type === 'single_choice') {
        // For single choice questions, display as a pie chart
        chartData = Object.entries(answerDistribution)
          .filter(([name, value]) => Number(value) > 0) // Only show options with responses
          .map(([name, value]: [string, any], index) => {
            // Clean up option names that might have issues
            let cleanName = name;
            if (name === 'Sejauh manakah anda bersetuju dengan kenyataan berikut: \'Saya yakin untuk mengaplikasikan kemahiran STI dalam situasi sebenar.\'') {
              cleanName = 'Question text (error)';
            }
            return {
              name: cleanName,
              value: value as number,
              fill: COLORS[index % COLORS.length] // Use consistent colors
            };
          });
      } else if (question.type === 'multiple_choice') {
        // For multiple choice questions, display as a bar chart
        chartData = Object.entries(answerDistribution)
          .filter(([name, value]) => Number(value) > 0) // Only show selected options
          .map(([name, value]: [string, any], index) => ({
            name: name, // API now handles quote removal
            value: value as number,
            fill: COLORS[index % COLORS.length] // Use consistent colors
          }))
          .sort((a, b) => b.value - a.value); // Sort by count (most selected first)
      } else {
        // For text questions or others
        chartData = Object.entries(answerDistribution)
      ...
      setQuestionDetail({
        question,
        chartData,
        filteredCount: questionAnswers.length
      });
    }
  };
  ...
  // Function to filter respondents by selected criteria across all filter types
  const getFilteredRespondents = () => {
    // Start with all respondents from raw data
    return filteredResponses.reduce((counts: Record<string, Record<string, number>>, respondent: any) => {
      // Track gender counts
      if (!counts.gender) counts.gender = {};
      const gender = respondent.gender || 'Unknown';
      counts.gender[gender] = (counts.gender[gender] || 0) + 1;
      
      // Track age group counts
      if (!counts.age) counts.age = {};
      const ageGroup = getAgeGroupFromValue(respondent.age);
      counts.age[ageGroup] = (counts.age[ageGroup] || 0) + 1;
      
      // Track education level counts
      if (!counts.education) counts.education = {};
      const eduLevel = respondent.edu_level || 'Unknown';
      counts.education[eduLevel] = (counts.education[eduLevel] || 0) + 1;
      
      // Track state counts
      if (!counts.state) counts.state = {};
      const state = respondent.state || 'Unknown';
  ...
  const filteredDemographics = getFilteredRespondents();
  
  // Prepare demographics data with cross-filtering applied
  const genderData = filteredDemographics.gender
    ? Object.entries(filteredDemographics.gender).map(([name, value]: [string, any], index) => ({
        name,
        value: value as number,
        fill: COLORS[index % COLORS.length]
      }))
  ...
  const ageData = filteredDemographics.age
    ? Object.entries(filteredDemographics.age).map(([name, value]: [string, any], index) => ({
        name,
        value: value as number,
        fill: COLORS[index % COLORS.length]
      })).sort((a, b) => {
  ...
  const educationData = filteredDemographics.education
    ? Object.entries(filteredDemographics.education).map(([name, value]: [string, any], index) => ({
        name,
        value: value as number,
        fill: COLORS[index % COLORS.length]
      }))
  ...
  const stateData = filteredDemographics.state
    ? Object.entries(filteredDemographics.state).map(([name, value]: [string, any], index) => ({
        name,
        value: value as number,
        fill: COLORS[index % COLORS.length]
      }))
            ...
            <div className="col-span-1 md:col-span-2 mt-8">
              ...
              <div className="grid grid-cols-1 gap-6">
                {results?.questions?.map((question: any, qIndex: number) => {
                  // Process question answers based on current filters
                  // Get question key (q1, q2, q3, etc.) based on question index
                  const qKey = `q${question.questionId}`;  // API uses q1, q2, q3 format
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                              ...
                              >
                                ...
                                <Bar 
                                  ...
                                  name="Responses"
                                  onClick={(data) => {
                                    // Create filter key for this question
                                    const filterKey = `q${question.questionId}`;
                                    ...
                                    setFilters(prevFilters => {
                                      ...
                                      if (currentAnswers[filterKey] === data.name) {
                                        // If already filtered by this answer, remove it
                                        delete currentAnswers[filterKey];
                                      } else {
                                        // Otherwise add/update the filter
                                        currentAnswers[filterKey] = data.name;
                                      }
                                ...
                                >
                                  {chartData.map((entry, index) => {
                                    ...
                                    const filterKey = `q${question.questionId}`;
                                    const isHighlighted = filters.answers[filterKey] === entry.name;
                                    
                                    return (
                                      <Cell 
                                        key={`${question.questionId}-${index}`} 
                                        fill={entry.fill}
                                        opacity={Object.keys(filters.answers).length > 0 ? 
