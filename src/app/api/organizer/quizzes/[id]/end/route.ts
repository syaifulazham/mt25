...
// POST /api/organizer/quizzes/[id]/end
// End a published quiz
export async function POST(
  ...
  try {
    ...
    // Only allow ending quizzes in "published" state
    if (existingQuiz.status !== "published") {
      return NextResponse.json(
        { error: "Only published quizzes can be ended" },
        { status: 400 }
      );
    }

    // Update the quiz status to ended
    ...
    console.error("[API] Error ending quiz:", error);
