...
export async function POST(
  ...
  try {
    ...
    console.log(`Received ${teamIds.length} team IDs for approval`);
    ...
    // Get event contest teams to update
    ...
    for (const ect of eventcontestTeams) {
      ...
      const result = await prisma.eventcontestteam.update({
        where: {
          id: ect.id
        },
        data: {
    ...
    return NextResponse.json(
      { 
        error: "Failed to approve teams and generate Excel file",
